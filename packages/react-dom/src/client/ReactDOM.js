...
function ReactRoot(
  ...
  const root = DOMRenderer.createContainer(container, isConcurrent, hydrate);   //此处root是一个FiberRoot
  this._internalRoot = root;
}
ReactRoot.prototype.render = function(
  children: ReactNodeList,   //ReactDom.render(<App />, container, callback)，children即<App/>，一个ReactElement
  callback: ?() => mixed,
): Work {
  const root = this._internalRoot;  //此处root是一个FiberRoot
  const work = new ReactWork();
...
function getReactRootElementInContainer(container: any) {
  ...
  // DOCUMENT_NODE 值是9，代表整个文档（DOM树的根节点html）
...
function shouldHydrateDueToLegacyHeuristic(container) {
  ...
  // true: rootElement存在 && rootElement是元素节点 && rootElement有data-reactroot属性
  return !!(
    rootElement &&
...
function legacyCreateRootFromDOMContainer(
  ...
  forceHydrate: boolean,  //是否复用已有的dom节点，服务端渲染（hydrate）是true，客户端渲染（render）为false
): Root {
  const shouldHydrate =
    forceHydrate || shouldHydrateDueToLegacyHeuristic(container);
  // First clear any existing content.
  if (!shouldHydrate) {   //如果是客户端渲染，则清空container的所有子节点
    let warned = false;
    let rootSibling;
...
function legacyRenderSubtreeIntoContainer(
  ...
  if (!root) {
    // Initial mount
    root = container._reactRootContainer = legacyCreateRootFromDOMContainer(   //此函数返回结果是一个ReactRoot
      container,
      forceHydrate,
    );
