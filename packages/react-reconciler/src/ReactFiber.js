...
function FiberNode(
  ...
  this.firstContextDependency = null;   // 一个列表，存放这个Fiber依赖的context
  ...
  this.expirationTime = NoWork;   // 过期事件，与优先级相关，代表任务在未来的哪个时间点应该被完成，不包括他的子树产生的任务
  this.childExpirationTime = NoWork;   // 快速确定子树中是否有不在等待的变化

  this.alternate = null;   // 在Fiber树更新的过程中，每个Fiber都会有一个跟其对应的Fiber，current <==> workInProgress 的对应关系，在渲染完成之后他们会交换位置
